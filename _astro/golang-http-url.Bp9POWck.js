import{s as F,g as c}from"./_astro_assets.D_IwRenL.js";import{c as y,r as h,m as d}from"./render-template.DGo9y8-J.js";import{u}from"./hoisted.CD8WJph8.js";import"./astro/assets-service.Cg1E-ILE.js";const f={src:"/_astro/Phenomenon.BPkEEYIn.png",width:946,height:644,format:"png"},g={src:"/_astro/architecture.BFLyRN88.png",width:2032,height:737,format:"png"},m=async function(t){const s={};{const o=new RegExp('__ASTRO_IMAGE_="([^"]*\\./img/golang-http-url/Phenomenon\\.png[^"]*)"',"g");let a,n=0;for(;(a=o.exec(t))!==null;){const e="./img/golang-http-url/Phenomenon.png_"+n,l=JSON.parse(a[1].replace(/&#x22;/g,'"')),{src:r,...p}=l;s[e]=await c({src:f,...p}),n++}}{const o=new RegExp('__ASTRO_IMAGE_="([^"]*\\./img/golang-http-url/architecture\\.png[^"]*)"',"g");let a,n=0;for(;(a=o.exec(t))!==null;){const e="./img/golang-http-url/architecture.png_"+n,l=JSON.parse(a[1].replace(/&#x22;/g,'"')),{src:r,...p}=l;s[e]=await c({src:g,...p}),n++}}return s};async function C(t){return m(t).then(s=>t.replaceAll(/__ASTRO_IMAGE_="([^"]+)"/gm,(o,a)=>{const n=JSON.parse(a.replace(/&#x22;/g,'"')),e=n.src+"_"+n.index;s[e].srcSet&&s[e].srcSet.values.length>0&&(s[e].attributes.srcset=s[e].srcSet.attribute);const{index:l,...r}=s[e].attributes;return F({src:s[e].src,...r})}))}const i=await C(`<h2 id="a-fun-bug-with-golangs-httpurl">A Fun “Bug” with Golang’s http.URL</h2>
...
<pre class="astro-code dracula" style="background-color:#282A36;color:#F8F8F2; overflow-x: auto; white-space: pre-wrap; word-wrap: break-word;" tabindex="0" data-language="text"><code><span class="line"><span>2024-02-20T01:53:41Z INFO - 10.233.64.42 "HEAD /files/qsdizt-1703128686-ca6addedebab4017/sd-train.json" 404</span></span>
<span class="line"><span>2024-02-20T01:53:41Z INFO - 10.233.64.42 "MOVE /files/qsdizt-1703128686-ca6addedebab4017/sd-tra" 404</span></span>
...
<pre class="astro-code dracula" style="background-color:#282A36;color:#F8F8F2; overflow-x: auto; white-space: pre-wrap; word-wrap: break-word;" tabindex="0" data-language="rust"><code><span class="line"><span style="color:#FF79C6">pub</span><span style="color:#FF79C6"> type</span><span style="color:#8BE9FD;font-style:italic"> Request</span><span style="color:#FF79C6"> =</span><span style="color:#F8F8F2"> hyper</span><span style="color:#FF79C6">::</span><span style="color:#8BE9FD;font-style:italic">Request</span><span style="color:#F8F8F2">&#x3C;</span><span style="color:#8BE9FD;font-style:italic">Incoming</span><span style="color:#F8F8F2">>;</span></span>
...
<pre class="astro-code dracula" style="background-color:#282A36;color:#F8F8F2; overflow-x: auto; white-space: pre-wrap; word-wrap: break-word;" tabindex="0" data-language="yaml"><code><span class="line"><span style="color:#FF79C6">-</span><span style="color:#8BE9FD"> https</span><span style="color:#FF79C6">:</span><span style="color:#BD93F9"> false</span></span>
...
<pre class="astro-code dracula" style="background-color:#282A36;color:#F8F8F2; overflow-x: auto; white-space: pre-wrap; word-wrap: break-word;" tabindex="0" data-language="text"><code><span class="line"><span>Note that the Path field is stored in decoded form: /%47%6f%2f becomes /Go/. A consequence is that it is impossible to tell which slashes in the Path were slashes in the raw URL and which were %2f. This distinction is rarely important, but when it is, the code should use the URL.EscapedPath method, which preserves the original encoding of Path.</span></span>
<span class="line"><span></span></span>
<span class="line"><span>The RawPath field is an optional field which is only set when the default encoding of Path is different from the escaped path. See the EscapedPath method for more details.</span></span>
<span class="line"><span></span></span></code></pre>
...
need the original encoding of the URL.</p>`),w={title:`A Fun "Bug" with Golang's http.URL`,publishDate:"20 February 2024",description:"Documenting an interesting Online issue troubleshooting journey",tags:["Golang","Network"],minutesRead:"4 min read"},B="/home/runner/work/lokiwager.github.io/lokiwager.github.io/src/content/post/golang-http-url.md",x=void 0;function D(){return`
...
`}function I(){return i}function q(){return[{depth:2,slug:"a-fun-bug-with-golangs-httpurl",text:"A Fun “Bug” with Golang’s http.URL"},{depth:3,slug:"prepare-the-minimal-environment",text:"Prepare the minimal environment"},{depth:3,slug:"the-root-cause",text:"The root cause"}]}const A=y((t,s,o)=>{const{layout:a,...n}=w;return n.file=B,n.url=x,h`${d()}${u(i)}`});export{A as Content,I as compiledContent,A as default,B as file,w as frontmatter,q as getHeadings,D as rawContent,x as url};
